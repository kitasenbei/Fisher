  ...
  return (
    <div className={`flex flex-col bg-[#2b2b2b] ${className}`}>
      ...
      <div className="flex items-center gap-1 px-1.5 py-1 border-b border-[#1e1e1e]">
        ...
        />
        {filterIcon && (
          <button className="text-[#999999] hover:text-[#cccccc] cursor-default shrink-0 flex items-center">
            {filterIcon}
          </button>
        )}
      </div>
      ...
      <div className="flex-1 overflow-y-auto">
        <Tree nodes={nodes} selected={selected} onSelect={onSelect} className="!border-0 !rounded-none" />
      </div>
    </div>
  )
}
