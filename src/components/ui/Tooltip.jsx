...
export default function Tooltip({ children, content, shortcut, description, position = 'top', className = '' }) {
  ...
  return (
    <div className={`relative inline-flex ${className}`} onMouseEnter={handleEnter} onMouseLeave={handleLeave}>
      {children}
      {show && (
        <div className={`absolute z-50 bg-[#1e1e1e] border border-[#3b3b3b] rounded-[3px] shadow-lg shadow-black/40 pointer-events-none ${isRich ? 'px-2.5 py-1.5 min-w-[140px]' : 'px-2 py-1'} ${posClasses[position]}`}>
