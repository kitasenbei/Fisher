...
import {
  Move, RotateCcw, Maximize, MousePointer2, Play, Pause, SkipBack, SkipForward,
  ChevronsLeft, ChevronsRight, Search, Filter,
  Box, Circle, Lightbulb, FolderOpen, Eye, Lock, Pin,
  Video, Crosshair, Hand, Grid3x3, Pen, Ruler, Scissors, Pencil,
  Wrench, Magnet, Layers, Database, Gem, Zap, Camera, Plus, Minus,
  Copy, Download, Sun, Cloud, Droplets, Wind, Flame, Snowflake,
  Music, Image, Film, Globe, Monitor, Speaker, Mic,
  Package, XCircle,
} from 'lucide-react'
import {
  Button, Input, Textarea, Select, Card, Modal, Badge, Avatar,
  Checkbox, Radio, Toggle, Slider, NumberInput, VectorInput, ColorPicker,
  Tooltip, ToastProvider, useToast, Progress, Skeleton,
  ContextMenu, useContextMenu, DropdownMenu, Dialog,
  Tree, Breadcrumb, MenuBar, Toolbar, StatusBar, PropertyPanel, PropertyRow,
  SegmentedControl, WorkspaceTabs, ToolStrip, IconTabStrip,
  ViewportOverlay, GizmoStrip, TransportControls, Outliner,
  IconToggle, KeyframeDot, Scrollbar,
} from './components'
...
function App() {
  ...
  return (
    <div className="flex flex-col h-screen bg-[#303030] text-[#cccccc] overflow-hidden">
      ...
      <div className="flex items-center bg-[#3c3c3c] border-b border-[#1e1e1e] h-6 px-1.5 gap-2 text-[11px]">
        ...
        <div className="flex gap-2 text-[#999999]">
          {['View', 'Select', 'Add', 'Object', 'Physics'].map((m) => (
            <button key={m} className="hover:text-[#cccccc] cursor-default">{m}</button>
          ))}
        </div>
        ...
        <div className="flex gap-0.5 ml-2">
          <Tooltip content="Snap to Grid" shortcut="Ctrl"><button className="p-0.5 text-[#999999] hover:text-[#cccccc] cursor-default"><Magnet size={12} /></button></Tooltip>
          <Tooltip content="Proportional Editing" shortcut="O"><button className="p-0.5 text-[#999999] hover:text-[#cccccc] cursor-default"><Circle size={12} /></button></Tooltip>
        </div>
      </div>
      ...
      <div className="flex-1 flex overflow-hidden">
          ...
          <div className="w-10 shrink-0 bg-[#303030] border-r border-[#1e1e1e]">
            <ToolStrip
              ...
              tools={[
                ...
                { id: 'select', icon: <MousePointer2 {...IT} />, label: 'Select Box', description: 'Select objects by dragging a box', shortcut: 'W' },
                { separator: true },
                { id: 'move', icon: <Move {...IT} />, label: 'Move', description: 'Move selected items', shortcut: 'G' },
                { id: 'rotate', icon: <RotateCcw {...IT} />, label: 'Rotate', description: 'Rotate selected items', shortcut: 'R' },
                { id: 'scale', icon: <Maximize {...IT} />, label: 'Scale', description: 'Scale (resize) selected items', shortcut: 'S' },
                { separator: true },
                { id: 'annotate', icon: <Pen {...IT} />, label: 'Annotate', description: 'Draw annotations on the viewport' },
                { id: 'measure', icon: <Ruler {...IT} />, label: 'Measure', description: 'Measure distances and angles' },
                { id: 'knife', icon: <Scissors {...IT} />, label: 'Knife', description: 'Cut through geometry', shortcut: 'K' },
                { separator: true },
          ...
          <div className="w-[220px] shrink-0 bg-[#2b2b2b] border-r border-[#1e1e1e] flex flex-col">
              ...
              {['Scene', 'Assets', 'Layers'].map((tab, i) => (
                <button
                  ...
                  className={`px-3 py-1.5 text-[10px] cursor-default ${
                    i === 0 ? 'bg-[#2b2b2b] text-[#cccccc] border-b border-[#2d8ceb]' : 'text-[#999999] hover:text-[#cccccc]'
                  }`}
                >
                  {tab}
                </button>
            ...
            <div className="flex-1 overflow-y-auto">
              <Tree
                ...
                nodes={[
                  { id: 'scene', label: 'Main Scene', icon: <Globe size={11} strokeWidth={1.5} />, expanded: true, children: [
                    { id: 'env', label: 'Environment', icon: <Sun size={11} strokeWidth={1.5} />, expanded: true, children: [
                      { id: 'sky', label: 'Sky Dome', icon: <Cloud size={11} strokeWidth={1.5} />, actions: rowActions },
                      { id: 'sun', label: 'Sun Light', icon: <Sun size={11} strokeWidth={1.5} />, actions: rowActions },
                      { id: 'fog', label: 'Volumetric Fog', icon: <Wind size={11} strokeWidth={1.5} />, actions: rowActions },
                    ]},
                    { id: 'geo', label: 'Geometry', icon: <FolderOpen size={11} strokeWidth={1.5} />, expanded: true, children: [
                      { id: 'cube', label: 'Cube', icon: <Box size={11} strokeWidth={1.5} />, actions: rowActions },
                      { id: 'sphere', label: 'Sphere', icon: <Circle size={11} strokeWidth={1.5} />, actions: rowActions },
                      { id: 'plane', label: 'Ground Plane', icon: <Minus size={11} strokeWidth={1.5} />, actions: rowActions },
                      { id: 'torus', label: 'Torus', icon: <Circle size={11} strokeWidth={1.5} />, actions: rowActions },
                      { id: 'cylinder', label: 'Cylinder', icon: <Database size={11} strokeWidth={1.5} />, actions: rowActions },
                    ]},
                    { id: 'lights', label: 'Lights', icon: <Lightbulb size={11} strokeWidth={1.5} />, children: [
                      { id: 'point1', label: 'Point Light', icon: <Lightbulb size={11} strokeWidth={1.5} />, actions: rowActions },
                      { id: 'spot1', label: 'Spot Light', icon: <Flame size={11} strokeWidth={1.5} />, actions: rowActions },
                      { id: 'area1', label: 'Area Light', icon: <Monitor size={11} strokeWidth={1.5} />, actions: rowActions },
                    ]},
                    { id: 'cameras', label: 'Cameras', icon: <Video size={11} strokeWidth={1.5} />, children: [
                      { id: 'cam1', label: 'Main Camera', icon: <Camera size={11} strokeWidth={1.5} />, actions: rowActions },
                      { id: 'cam2', label: 'Cinematic Cam', icon: <Film size={11} strokeWidth={1.5} />, actions: rowActions },
                    ]},
                    { id: 'fx', label: 'Effects', icon: <Zap size={11} strokeWidth={1.5} />, children: [
                      { id: 'particle1', label: 'Fire Particles', icon: <Flame size={11} strokeWidth={1.5} />, actions: rowActions },
                      { id: 'particle2', label: 'Rain Drops', icon: <Droplets size={11} strokeWidth={1.5} />, actions: rowActions },
                      { id: 'particle3', label: 'Snow', icon: <Snowflake size={11} strokeWidth={1.5} />, actions: rowActions },
                    ]},
                    { id: 'audio', label: 'Audio', icon: <Music size={11} strokeWidth={1.5} />, children: [
            ...
            <div className="flex items-center gap-1 px-1.5 py-1 border-t border-[#1e1e1e]">
              <Tooltip content="Add Object"><Button variant="ghost" size="sm"><Plus size={11} /></Button></Tooltip>
              <Tooltip content="Delete"><Button variant="ghost" size="sm"><Minus size={11} /></Button></Tooltip>
              <Tooltip content="Duplicate"><Button variant="ghost" size="sm"><Copy size={11} /></Button></Tooltip>
              <div className="flex-1" />
              <Tooltip content="Filter"><Button variant="ghost" size="sm"><Filter size={11} /></Button></Tooltip>
            </div>
          </div>
        </div>
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          <div className="flex-1 bg-[#353535] relative" onContextMenu={ctx.onContextMenu}>
            <ViewportOverlay info="User Perspective" subInfo={`(1) Main Scene | ${treeSelected.charAt(0).toUpperCase() + treeSelected.slice(1)}`} className="absolute top-2 left-3" />
            ...
            <ContextMenu pos={ctx.pos} onClose={ctx.close} items={[
                      ...
                      <div className="flex-1 h-full relative">
                        <div className="absolute inset-y-0 left-0 right-0 flex items-center gap-[2px]">
                          {Array.from({ length: 5 }, (_, i) => (
                            <div key={i} className="flex items-center" style={{ marginLeft: `${(Math.random() * 80 + 10)}%`, position: 'absolute' }}>
            ...
            {activeBottomTab === 'output' && (
              <div className="flex-1 p-2">
                ...
                <div className="mt-2 grid grid-cols-3 gap-2">
                  <div><Progress label="Geometry" value={100} /></div>
                  <div><Progress label="Lighting" value={85} /></div>
                  <div><Progress label="Post FX" value={40} /></div>
                </div>
        ...
        <div className="w-[300px] shrink-0 flex flex-col border-l border-[#1e1e1e] overflow-hidden">
          ...
          <Outliner
            ...
            nodes={[
              { id: 's1', label: 'Scene Collection', icon: <FolderOpen {...IS} />, expanded: true, children: [
                { id: 'c1', label: 'Collection', icon: <Layers {...IS} />, expanded: true, actions: <IconToggle icon={<Eye size={10} strokeWidth={1.5} />} checked />, children: [
                ...
                <>
                  ...
                  <PropertyPanel title="Transform" draggable onToggle={() => {}}>
                    <div className="space-y-1">
                      <PropertyRow label="Location" actions={<><IconToggle icon={<Lock size={9} strokeWidth={1.5} />} tooltip="Lock" /><KeyframeDot active /></>}>
                        <VectorInput value={pos} step={0.1} onChange={setPos} />
                      </PropertyRow>
                      <PropertyRow label="Rotation" actions={<><IconToggle icon={<Lock size={9} strokeWidth={1.5} />} tooltip="Lock" /><KeyframeDot /></>}>
                        <VectorInput value={rot} step={1} onChange={setRot} />
                      </PropertyRow>
                      <PropertyRow label="Scale" actions={<><IconToggle icon={<Lock size={9} strokeWidth={1.5} />} tooltip="Lock" /><KeyframeDot /></>}>
                        <VectorInput value={scl} step={0.1} onChange={setScl} />
                      </PropertyRow>
                      <PropertyRow label="Mode">
                        <Select id="rot-mode2" options={[
                  ...
                  <PropertyPanel title="Viewport Display" draggable onToggle={() => {}}>
                    <PropertyRow label="Color"><ColorPicker value={col1} onChange={(e) => setCol1(e.target.value)} /></PropertyRow>
                    <PropertyRow label="Display As">
                      <Select id="disp2" options={[
                        { value: 'solid', label: 'Solid' },
                    ...
                    <PropertyRow label="Wireframe"><Slider value={opacity} max={100} onChange={(e) => setOpacity(e.target.value)} /></PropertyRow>
                    <PropertyRow label="Shadows"><Checkbox id="cast2" label="Cast" checked /></PropertyRow>
                    <PropertyRow label=""><Checkbox id="recv2" label="Receive" checked /></PropertyRow>
                  </PropertyPanel>

                  {/* Relations */}
                  <PropertyPanel title="Relations" draggable onToggle={() => {}}>
                    <PropertyRow label="Parent"><span className="text-[11px] text-[#666666]">None</span></PropertyRow>
                    <PropertyRow label="Collection"><span className="text-[11px] text-[#cccccc]">Collection</span></PropertyRow>
                    <PropertyRow label="Pass Index"><NumberInput id="pidx" value={0} min={0} step={1} /></PropertyRow>
                  </PropertyPanel>
                  ...
                  <PropertyPanel title="Visibility" draggable onToggle={() => {}}>
                    ...
                    <PropertyRow label="Selectable"><Checkbox id="vis-sel2" checked /></PropertyRow>
                    <PropertyRow label="Holdout"><Checkbox id="holdout" /></PropertyRow>
                    <PropertyRow label="Shadow Catcher"><Checkbox id="shadow-catch" /></PropertyRow>
                  </PropertyPanel>
                </>
              )}
                ...
                <>
                  <PropertyPanel title="Surface" draggable onToggle={() => {}}>
                    <PropertyRow label="Shader">
                      <div className="space-y-1.5">
                        <Radio id="sh-pbr" name="shader2" value="pbr" label="PBR Standard" checked={radio === 'pbr'} onChange={() => setRadio('pbr')} />
                        <Radio id="sh-unlit" name="shader2" value="unlit" label="Unlit" checked={radio === 'unlit'} onChange={() => setRadio('unlit')} />
                        <Radio id="sh-toon" name="shader2" value="toon" label="Toon" checked={radio === 'toon'} onChange={() => setRadio('toon')} />
                      </div>
                    </PropertyRow>
                    <PropertyRow label="Base Color"><ColorPicker value={col1} onChange={(e) => setCol1(e.target.value)} /></PropertyRow>
                    <PropertyRow label="Roughness" actions={<KeyframeDot />}><Slider value={roughness} onChange={(e) => setRoughness(e.target.value)} /></PropertyRow>
                    <PropertyRow label="Metallic" actions={<KeyframeDot />}><Slider value={metallic} onChange={(e) => setMetallic(e.target.value)} /></PropertyRow>
                    <PropertyRow label="Emission"><Slider value={emission} onChange={(e) => setEmission(e.target.value)} /></PropertyRow>
                    <PropertyRow label="Emission Color"><ColorPicker value={col3} onChange={(e) => setCol3(e.target.value)} /></PropertyRow>
                    <PropertyRow label="Opacity" actions={<KeyframeDot />}><Slider value={opacity} max={100} onChange={(e) => setOpacity(e.target.value)} /></PropertyRow>
                    <PropertyRow label="Alpha Clip"><Toggle /></PropertyRow>
                  </PropertyPanel>

                  <PropertyPanel title="Normal Map" draggable onToggle={() => {}}>
                    <PropertyRow label="Texture"><span className="text-[11px] text-[#666666]">None</span></PropertyRow>
                    <PropertyRow label="Strength"><Slider value={100} /></PropertyRow>
                    <PropertyRow label="Flip Y"><Checkbox id="flip-y" /></PropertyRow>
                  </PropertyPanel>
                </>
              )}
                ...
                <>
                  <PropertyPanel title="Rigid Body" draggable onToggle={() => {}}>
                    <PropertyRow label="Type">
                      <Select id="rb-type" options={[
                        { value: 'active', label: 'Active' },
                        { value: 'passive', label: 'Passive' },
                        { value: 'kinematic', label: 'Kinematic' },
                      ]} />
                    </PropertyRow>
                    <PropertyRow label="Mass"><NumberInput id="rb-mass" value={mass} min={0} step={0.1} onChange={(e) => setMass(e.target.value)} /></PropertyRow>
                    <PropertyRow label="Friction"><Slider value={friction} onChange={(e) => setFriction(e.target.value)} /></PropertyRow>
                    <PropertyRow label="Bounciness"><Slider value={bounce} onChange={(e) => setBounce(e.target.value)} /></PropertyRow>
                    <PropertyRow label="Gravity"><Toggle checked /></PropertyRow>
                    <PropertyRow label="Collision">
                      <Select id="col-shape" options={[
                        { value: 'box', label: 'Box' },
                  ...
                  <PropertyPanel title="Fluid" draggable onToggle={() => {}}>
                    <PropertyRow label="Enable"><Toggle /></PropertyRow>
                    <PropertyRow label="Density"><NumberInput id="fl-density" value={density} step={10} onChange={(e) => setDensity(e.target.value)} /></PropertyRow>
                    <PropertyRow label="Viscosity"><Slider value={10} /></PropertyRow>
                    <PropertyRow label="Velocity"><VectorInput value={vel} step={0.1} onChange={setVel} /></PropertyRow>
                  </PropertyPanel>
                </>
              )}
                ...
                <>
                  <PropertyPanel title="Render Engine" draggable onToggle={() => {}}>
                    <PropertyRow label="Engine">
                      <Select id="render-engine" options={[
                        { value: 'fisher', label: 'Fisher RT' },
                        { value: 'pathtracer', label: 'Path Tracer' },
                        { value: 'raster', label: 'Rasterizer' },
                      ]} />
                    </PropertyRow>
                    <PropertyRow label="Samples"><NumberInput id="samples" value={128} min={1} max={4096} step={1} /></PropertyRow>
                    <PropertyRow label="Denoise"><Toggle checked /></PropertyRow>
                  </PropertyPanel>

                  <PropertyPanel title="Camera" draggable onToggle={() => {}}>
                    <PropertyRow label="FOV"><Slider value={fov} min={10} max={120} onChange={(e) => setFov(e.target.value)} /></PropertyRow>
                    <PropertyRow label="Near Clip"><NumberInput id="near" value={near} min={0.001} step={0.01} onChange={(e) => setNear(e.target.value)} /></PropertyRow>
                    <PropertyRow label="Far Clip"><NumberInput id="far" value={far} min={1} step={10} onChange={(e) => setFar(e.target.value)} /></PropertyRow>
                    <PropertyRow label="Depth of Field"><Toggle /></PropertyRow>
                    <PropertyRow label="Aperture"><Slider value={22} max={100} /></PropertyRow>
                  </PropertyPanel>

                  <PropertyPanel title="Color Management" draggable onToggle={() => {}}>
                    <PropertyRow label="Exposure"><Slider value={brightness} onChange={(e) => setBrightness(e.target.value)} /></PropertyRow>
                    <PropertyRow label="Gamma"><Slider value={50} /></PropertyRow>
                    <PropertyRow label="Temperature"><NumberInput id="temp" value={temperature} min={1000} max={12000} step={100} onChange={(e) => setTemperature(e.target.value)} /></PropertyRow>
                    <PropertyRow label="Look">
                      <Select id="look" options={[
                  ...
                  <PropertyPanel title="Output" draggable onToggle={() => {}}>
                    <PropertyRow label="Resolution"><VectorInput value={{ x: 1920, y: 1080 }} axes={['x', 'y']} step={1} /></PropertyRow>
                    ...
                    <PropertyRow label="Quality"><Slider value={90} /></PropertyRow>
                  </PropertyPanel>
                </>
              )}
              ...
              {activePropTab === 'scene' && (
                <>
                  <PropertyPanel title="Scene" draggable onToggle={() => {}}>
                    <PropertyRow label="Name"><Input id="scene-name" placeholder="Main Scene" className="w-full bg-[#383838] text-[#cccccc] text-[11px] px-2 py-0.5 border border-[#3b3b3b] rounded-[3px] outline-none focus:border-[#2d8ceb]" /></PropertyRow>
                    <PropertyRow label="Camera">
                      <Select id="scene-cam" icon={<Camera {...IS} />} options={[
                  ...
                  <PropertyPanel title="Units" draggable onToggle={() => {}}>
                    ...
                    <PropertyRow label="Scale"><NumberInput id="unit-scale" value={1} min={0.001} step={0.001} /></PropertyRow>
                  </PropertyPanel>

                  <PropertyPanel title="Gravity" draggable onToggle={() => {}}>
                    <PropertyRow label="Enable"><Checkbox id="grav-enable" label="Active" checked /></PropertyRow>
                    <PropertyRow label="Direction"><VectorInput value={{ x: 0, y: -9.81, z: 0 }} step={0.01} /></PropertyRow>
                  </PropertyPanel>

                  <PropertyPanel title="Audio" draggable onToggle={() => {}}>
                    <PropertyRow label="Volume"><Slider value={volume} onChange={(e) => setVolume(e.target.value)} /></PropertyRow>
                    <PropertyRow label="Distance Model">
                      <Select id="audio-dist" options={[
                        { value: 'linear', label: 'Linear' },
                        { value: 'inverse', label: 'Inverse' },
                        { value: 'expo', label: 'Exponential' },
                      ]} />
                    </PropertyRow>
                    <PropertyRow label="Doppler"><Toggle checked /></PropertyRow>
                  </PropertyPanel>
                </>
              )}
      ...
      <StatusBar
        left={[
          <span key="s" className="text-[#cccccc]">Select</span>,
          'Rotate View',
          'Pan',
          'Object',
        ]}
      ...
      {/* Preferences Modal */}
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} className="bg-[#2b2b2b] border border-[#3b3b3b] rounded-[3px] p-0 shadow-lg shadow-black/40 backdrop:bg-black/50 w-[500px]">
        ...
        <div className="flex min-h-[300px]">
          <div className="w-[130px] bg-[#323232] border-r border-[#3b3b3b] py-1">
            {['Interface', 'Viewport', 'Keymap', 'System', 'Save & Load', 'Addons'].map((item, i) => (
              <button key={item} className={`w-full text-left px-3 py-1 text-[11px] cursor-default ${i === 0 ? 'bg-[#2d8ceb] text-white' : 'text-[#999999] hover:bg-[#404040]'}`}>
                {item}
              </button>
            ))}
          </div>
          <div className="flex-1 p-3 space-y-3 overflow-y-auto">
            <PropertyPanel title="Appearance" onToggle={() => {}}>
              <PropertyRow label="Theme">
                <Select id="pref-theme" options={[
                  { value: 'dark', label: 'Dark' },
                  { value: 'light', label: 'Light' },
                  { value: 'midnight', label: 'Midnight' },
                ]} />
              </PropertyRow>
              <PropertyRow label="Font Size"><NumberInput id="font-size" value={12} min={8} max={24} step={1} /></PropertyRow>
              <PropertyRow label="Accent"><ColorPicker value={col1} onChange={(e) => setCol1(e.target.value)} /></PropertyRow>
              <PropertyRow label="Animations"><Toggle checked /></PropertyRow>
              <PropertyRow label="Tooltips"><Toggle checked /></PropertyRow>
            </PropertyPanel>
            <PropertyPanel title="Viewport" onToggle={() => {}}>
              <PropertyRow label="Anti-alias"><Checkbox id="pref-aa" label="MSAA 4x" checked /></PropertyRow>
              <PropertyRow label="VSync"><Toggle checked /></PropertyRow>
              <PropertyRow label="FPS Limit"><NumberInput id="fps-limit" value={60} min={30} max={240} step={1} /></PropertyRow>
            </PropertyPanel>
          </div>
        </div>
      </Modal>
