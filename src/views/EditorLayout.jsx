...
import {
  ...
  Plus, Trash2, Download, Upload, Magnet, Volume2,
  MousePointer2, Hand, ZoomIn, Scan, Pencil, Eraser,
  Maximize2, AudioLines, SlidersHorizontal, List, Info,
  Eye, Grid3x3, Layers, Spline,
} from 'lucide-react'
import {
...
export default function EditorLayout() {
  ...
  const isStdMode = file?.parsed?.general?.mode === 0 || (file?.parsed?.general?.mode === undefined && file?.parsed?.hitObjects?.some(o => o.type === 'circle' || o.type === 'slider'))
  ...
  const handleKeyDown = useCallback((e) => {
    ...
    if (e.key === 'a' && (e.ctrlKey || e.metaKey)) { e.preventDefault(); dispatch('SELECT_ALL') }
    if (e.key === 'z' && (e.ctrlKey || e.metaKey) && !e.shiftKey) { e.preventDefault(); dispatch('UNDO') }
    if (e.key === 'z' && (e.ctrlKey || e.metaKey) && e.shiftKey) { e.preventDefault(); dispatch('REDO') }
    if (e.key === 's' && (e.ctrlKey || e.metaKey)) { e.preventDefault(); handleExport() }
      ...
      <MenuBar menus={[
        ...
        { label: 'Edit', items: [
          ...
          { label: 'Delete Selected', shortcut: 'Del', action: () => {
            if (selection.size > 0) { dispatch('REMOVE_POINTS', selection); toast(`Deleted ${selection.size} point(s)`, 'info') }
          }},
        ]},
        { label: 'SV', items: [
        ...
        { label: 'View', items: [
          ...
          { label: 'Reset Zoom', action: () => dispatch('SET_VIEWPORT', { startMs: 0, endMs: 10000, svMin: 0, svMax: 4 }) },
          { separator: true },
          ...(isStdMode ? [{ label: playfieldOpen ? 'Hide Playfield' : 'Show Playfield', action: () => setPlayfieldOpen(o => !o) }] : []),
        ]},
        { label: 'Help', items: [
          { label: 'Keyboard Shortcuts' },
          { label: 'About Fisher SV Editor' },
        ]},
      ]} />
      ...
      <div className="flex items-center bg-[#3c3c3c] border-b border-[#1e1e1e] h-6 px-2 gap-2 text-[11px]">
        ...
        <Tooltip content="Delete Selected">
          <Button variant="ghost" size="sm" onClick={() => {
            if (selection.size > 0) { dispatch('REMOVE_POINTS', selection); toast(`Deleted ${selection.size}`, 'info') }
          }}>
            <Trash2 size={11} />
          </Button>
        </Tooltip>
        ...
        <div className="flex-1 flex flex-col">
          <div className="flex-1 flex">
            ...
            {file ? (
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center gap-3">
                ...
                <Button variant="secondary" size="md" onClick={fileOpen}>
                  <Upload size={12} className="inline mr-1.5" />Open Files
                </Button>
              </div>
            )}
            ...
            {file && (
              <div className="absolute top-2 right-2 flex gap-1 items-start">
                <GizmoStrip
                  items={[
                    { icon: <Grid3x3 size={12} />, label: 'Beat Grid', active: state.display.beatGrid, action: () => dispatch('TOGGLE_DISPLAY', 'beatGrid') },
                    { icon: <Eye size={12} />, label: 'SV Lines', active: state.display.svLine, action: () => dispatch('TOGGLE_DISPLAY', 'svLine') },
                    { icon: <Spline size={12} />, label: 'Log Scale', active: state.display.logScale, action: () => dispatch('TOGGLE_DISPLAY', 'logScale') },
                    { icon: <Layers size={12} />, label: 'Display Settings', active: overlayOpen, action: () => setOverlayOpen(o => !o) },
                  ]}
              ...
              <div className="absolute top-10 right-2 w-[220px] bg-[#2b2b2b] border border-[#3b3b3b] rounded-[3px] shadow-lg shadow-black/40 text-[11px] z-10">
                <div className="px-2.5 py-1.5 border-b border-[#3b3b3b]">
                  ...
                  <div className="space-y-1">
                    <Checkbox id="ov-beatgrid" label="Beat Grid" checked={state.display.beatGrid} onChange={() => dispatch('TOGGLE_DISPLAY', 'beatGrid')} />
                    <Checkbox id="ov-svgrid" label="SV Grid Lines" checked={state.display.svTags} onChange={() => dispatch('TOGGLE_DISPLAY', 'svTags')} />
                    <Checkbox id="ov-measure" label="Measure Lines" checked={state.display.measureLines} onChange={() => dispatch('TOGGLE_DISPLAY', 'measureLines')} />
                  </div>
                </div>
                <div className="px-2.5 py-1.5 border-b border-[#3b3b3b]">
                  <div className="text-[10px] text-[#999999] mb-1">Elements</div>
                  <div className="space-y-1">
                    <Checkbox id="ov-svline" label="SV Step Line" checked={state.display.svLine} onChange={() => dispatch('TOGGLE_DISPLAY', 'svLine')} />
                    <Checkbox id="ov-svfill" label="SV Fill Area" checked={state.display.svFill} onChange={() => dispatch('TOGGLE_DISPLAY', 'svFill')} />
                    <Checkbox id="ov-svpoints" label="SV Points" checked={state.display.svPoints} onChange={() => dispatch('TOGGLE_DISPLAY', 'svPoints')} />
                    <Checkbox id="ov-bpmlabel" label="BPM Labels" checked={state.display.bpmLabels} onChange={() => dispatch('TOGGLE_DISPLAY', 'bpmLabels')} />
                    <Checkbox id="ov-playhead" label="Playhead" checked={state.display.playhead} onChange={() => dispatch('TOGGLE_DISPLAY', 'playhead')} />
                  </div>
                </div>
                <div className="px-2.5 py-1.5">
                  <div className="text-[10px] text-[#999999] mb-1">Labels</div>
                  <div className="space-y-1">
                    <Checkbox id="ov-svtags" label="SV Axis Tags" checked={state.display.svTags} onChange={() => dispatch('TOGGLE_DISPLAY', 'svTags')} />
                    <Checkbox id="ov-hoverlabel" label="Hover Labels" checked={state.display.hoverLabels} onChange={() => dispatch('TOGGLE_DISPLAY', 'hoverLabels')} />
                  </div>
                </div>
              </div>
            )}
            ...
            <div className="w-8 shrink-0 bg-[#303030] border-l border-[#1e1e1e]">
              <ToolStrip
                tools={[
                  { id: 'select', icon: <MousePointer2 size={14} strokeWidth={1.5} />, label: 'Select', description: 'Select & move points', shortcut: 'V' },
                  { id: 'pen', icon: <Pencil size={14} strokeWidth={1.5} />, label: 'Draw', description: 'Click to add SV points', shortcut: 'P' },
                  { id: 'eraser', icon: <Eraser size={14} strokeWidth={1.5} />, label: 'Erase', description: 'Click points to delete', shortcut: 'E' },
                  { separator: true },
                  { id: 'pan', icon: <Hand size={14} strokeWidth={1.5} />, label: 'Pan', description: 'Drag to pan viewport', shortcut: 'H' },
                  { id: 'zoom', icon: <ZoomIn size={14} strokeWidth={1.5} />, label: 'Zoom', description: 'Click to zoom in, Alt+click to zoom out', shortcut: 'Z' },
                  { id: 'zoom-area', icon: <Scan size={14} strokeWidth={1.5} />, label: 'Zoom Area', description: 'Drag a rectangle to zoom into', shortcut: 'Shift+Z' },
                  { separator: true },
                  { id: 'seek', icon: <AudioLines size={14} strokeWidth={1.5} />, label: 'Seek', description: 'Click to set playback position', shortcut: 'T' },
                  { separator: true },
                  { id: 'fit', icon: <Maximize2 size={14} strokeWidth={1.5} />, label: 'Fit All', description: 'Zoom to fit all points' },
                ]}
                value={activeTool}
          ...
          <div className="h-7 bg-[#2b2b2b] border-t border-[#1e1e1e] flex items-center px-2 gap-2">
            <TransportControls
              ...
              onPlay={() => { audio.play(playback.currentTimeMs); dispatch('SET_PLAYBACK', { playing: true }) }}
              onPause={() => { audio.pause(); dispatch('SET_PLAYBACK', { playing: false, currentTimeMs: audio.getCurrentTimeMs() }) }}
          ...
          {rightTab === 'info' && (
            <div className="flex-1 overflow-y-auto p-2 space-y-3">
              <PropertyPanel title="Beatmap">
                <PropertyRow label="Title"><span className="text-[11px] text-[#cccccc] truncate">{file?.parsed?.metadata?.title || '—'}</span></PropertyRow>
                <PropertyRow label="Artist"><span className="text-[11px] text-[#cccccc] truncate">{file?.parsed?.metadata?.artist || '—'}</span></PropertyRow>
                <PropertyRow label="Diff"><span className="text-[11px] text-[#cccccc] truncate">{file?.parsed?.metadata?.version || '—'}</span></PropertyRow>
                <PropertyRow label="Creator"><span className="text-[11px] text-[#cccccc] truncate">{file?.parsed?.metadata?.creator || '—'}</span></PropertyRow>
                <PropertyRow label="File"><span className="text-[11px] text-[#999999] truncate">{file?.filename || '—'}</span></PropertyRow>
              </PropertyPanel>
              <PropertyPanel title="Statistics">
                ...
                <PropertyRow label="Duration"><span className="text-[11px] text-[#cccccc]">{audio.duration ? `${(audio.duration / 1000 / 60).toFixed(1)} min` : '—'}</span></PropertyRow>
              </PropertyPanel>
            </div>
          )}
