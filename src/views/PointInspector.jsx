          ...
          {meta ? (
            <>
              <PropertyRow label="Title">
                <span className="text-[11px] text-[#cccccc] truncate">{meta.title || '—'}</span>
              </PropertyRow>
              <PropertyRow label="Artist">
                <span className="text-[11px] text-[#cccccc] truncate">{meta.artist || '—'}</span>
              </PropertyRow>
              <PropertyRow label="Diff">
                <span className="text-[11px] text-[#cccccc] truncate">{meta.version || '—'}</span>
              </PropertyRow>
              <PropertyRow label="Creator">
                <span className="text-[11px] text-[#cccccc] truncate">{meta.creator || '—'}</span>
              </PropertyRow>
            </>
          ) : (
      ...
      <div className="p-2 space-y-1">
        <PropertyPanel title={isSV ? 'SV Point' : 'BPM Point'}>
          ...
          <PropertyRow label="Volume">
            <Slider value={point.volume} min={0} max={100} onChange={(e) => update({ volume: e.target.value })} />
          </PropertyRow>
          <PropertyRow label="Kiai">
            <Toggle
              checked={!!(point.effects & 1)}
              onChange={(e) => {
    ...
    <div className="p-2 space-y-1">
      <PropertyPanel title={`${selectedPoints.length} Points Selected`}>
        ...
        <div className="text-[10px] text-[#666666] mt-2">
          {selectedPoints.filter((p) => !p.uninherited).length} SV, {selectedPoints.filter((p) => p.uninherited).length}{' '}
          BPM
        </div>
      </PropertyPanel>
    </div>
