...
function getEasedValue(t, easing) {
  switch (easing) {
    case 0:
      return t // Linear
    case 1:
      return t * (2 - t) // Easing Out (start fast, slow down)
    case 2:
      return t * t // Easing In (start slow, speed up)
    case 3:
      return t < 0.5 ? 2 * t * t : 1 - Math.pow(-2 * t + 2, 2) / 2 // Quad In/Out
    case 4:
      return t * t // Quad In
    case 5:
      return 1 - (1 - t) * (1 - t) // Quad Out
    case 6:
      return t < 0.5 ? 2 * t * t : 1 - Math.pow(-2 * t + 2, 2) / 2 // Quad In/Out
    case 7:
      return t * t * t // Cubic In
    case 8:
      return 1 - Math.pow(1 - t, 3) // Cubic Out
    case 9:
      return t < 0.5 ? 4 * t * t * t : 1 - Math.pow(-2 * t + 2, 3) / 2 // Cubic In/Out
    case 10:
      return t * t * t * t // Quart In
    case 11:
      return 1 - Math.pow(1 - t, 4) // Quart Out
    case 12:
      return t < 0.5 ? 8 * t * t * t * t : 1 - Math.pow(-2 * t + 2, 4) / 2 // Quart In/Out
    case 13:
      return t * t * t * t * t // Quint In
    case 14:
      return 1 - Math.pow(1 - t, 5) // Quint Out
    case 15:
      return t < 0.5 ? 16 * t * t * t * t * t : 1 - Math.pow(-2 * t + 2, 5) / 2 // Quint In/Out
    case 16:
      return 1 - Math.cos((t * Math.PI) / 2) // Sine In
    case 17:
      return Math.sin((t * Math.PI) / 2) // Sine Out
    case 18:
      return -(Math.cos(Math.PI * t) - 1) / 2 // Sine In/Out
    case 19:
      return t === 0 ? 0 : Math.pow(2, 10 * t - 10) // Expo In
    case 20:
      return t === 1 ? 1 : 1 - Math.pow(2, -10 * t) // Expo Out
    case 21:
      return t === 0 ? 0 : t === 1 ? 1 : t < 0.5 ? Math.pow(2, 20 * t - 10) / 2 : (2 - Math.pow(2, -20 * t + 10)) / 2 // Expo In/Out
    case 22:
      return 1 - Math.sqrt(1 - t * t) // Circ In
    case 23:
      return Math.sqrt(1 - Math.pow(t - 1, 2)) // Circ Out
    case 24:
      return t < 0.5 ? (1 - Math.sqrt(1 - Math.pow(2 * t, 2))) / 2 : (Math.sqrt(1 - Math.pow(-2 * t + 2, 2)) + 1) / 2 // Circ In/Out
    case 25: {
      // Elastic In
    ...
    case 30:
      return 2.70158 * t * t * t - 1.70158 * t * t // Back In
    case 31:
      return 1 + 2.70158 * Math.pow(t - 1, 3) + 1.70158 * Math.pow(t - 1, 2) // Back Out
    case 32: {
      // Back In/Out
    ...
    default:
      return t // Unknown easing, use linear
  }
}
...
function StoryboardRenderer({
  ...
  useEffect(() => {
    if (propWidth && propHeight) {
      sizeRef.current = { width: propWidth, height: propHeight }
      return
    }
  ...
  useEffect(() => {
    ...
    for (const color of uniqueColors) {
      ...
      ctx.beginPath()
      ctx.arc(cx, cy, r2 - borderW / 2, 0, Math.PI * 2)
      ...
      ctx.beginPath()
      ctx.arc(cx, cy, r2 - borderW - ringW / 2, 0, Math.PI * 2)
      ...
      if (innerR > 0) {
        ...
        ctx.lineWidth = innerR
        ctx.stroke()
      }
      // Glossy highlight
      ctx.beginPath()
      ctx.arc(cx - r2 * 0.1, cy - r2 * 0.15, r2 * 0.3, 0, Math.PI * 2)
      ctx.strokeStyle = 'rgba(255,255,255,0.12)'
      ctx.lineWidth = r2 * 0.3
      ctx.stroke()
  ...
  useEffect(() => {
    ...
    function rebuildActiveSet(time) {
      ...
      let lo = 0,
        hi = spritesByStart.length
      while (lo < hi) {
        const mid = (lo + hi) >> 1
        sortedSprites[spritesByStart[mid]].startTime <= time ? (lo = mid + 1) : (hi = mid)
      }
      startCursor = lo
      lo = 0
      hi = spritesByEnd.length
      while (lo < hi) {
        const mid = (lo + hi) >> 1
        sortedSprites[spritesByEnd[mid]].endTime < time ? (lo = mid + 1) : (hi = mid)
      }
      endCursor = lo
    ...
    function updateActiveSet(time) {
      ...
      while (startCursor < spritesByStart.length && sortedSprites[spritesByStart[startCursor]].startTime <= time) {
        ...
        if (sortedSprites[idx].endTime >= time) {
          activeSet.add(idx)
          changed = true
        }
        startCursor++
    ...
    const render = () => {
      ...
      for (let ai = 0; ai < activeDrawOrder.length; ai++) {
        ...
        while (cLo < cHi) {
          const mid = (cLo + cHi) >> 1
          cmds[mid].start_time <= time ? (cLo = mid + 1) : (cHi = mid)
        }
        ...
        for (let j = cLo; j < cmdLen; j++) {
          ...
          if (type === 'F' && !hasInitF) {
            alpha = params[0]
            hasInitF = true
          } else if (type === 'S' && !hasInitS) {
            uniformScale = params[0]
            hasInitS = true
          } else if (type === 'V' && !hasInitV) {
            vectorScaleX = params[0]
            vectorScaleY = params[1]
            hasInitV = true
          } else if (type === 'M' && !hasInitM) {
            x = params[0]
            y = params[1]
            hasInitM = true
          } else if (type === 'MX' && !hasInitMX) {
            x = params[0]
            hasInitMX = true
          } else if (type === 'MY' && !hasInitMY) {
            y = params[0]
            hasInitMY = true
          } else if (type === 'R' && !hasInitR) {
            rot = params[0]
            hasInitR = true
          } else if (type === 'C' && !hasInitC) {
            colorR = params[0] / 255
            colorG = params[1] / 255
            colorB = params[2] / 255
            hasInitC = true
      ...
      if (pf && pfData) {
        ...
        if (pfTp) {
          ...
          for (let i = pfTp.length - 1; i >= 0; i--) {
            if (pfTp[i].uninherited && pfTp[i].offset <= time) {
              beatLen = pfTp[i].msPerBeat
              beatOrigin = pfTp[i].offset
              break
            }
          }
          if (beatLen > 0) {
        ...
        let pfLo = 0,
          pfHi = objects.length
        while (pfLo < pfHi) {
          const mid = (pfLo + pfHi) >> 1
          objects[mid].fadeOutEnd < time ? (pfLo = mid + 1) : (pfHi = mid)
        }
        const pfVisible = []
        ...
        for (let v = pfVisible.length - 1; v >= 0; v--) {
          ...
          if (obj.type === 'slider') {
            ...
            if (sp && spt) {
              ...
              gl.uniform2f(
                gl.getUniformLocation(sp, 'u_translation'),
                pfOX + spt.minX * pfScale,
                pfOY + spt.minY * pfScale,
              )
          ...
          if ((obj.type === 'circle' || obj.type === 'slider') && cp) {
            ...
            if (currentProgram !== 'circle') {
              useCircleProgram()
              currentProgram = 'circle'
            }
          ...
          if (obj.type === 'slider' && time >= obj.time && time <= obj.hitTime && obj.sliderPath?.length >= 2) {
            ...
            const path = obj.sliderPath
            let target = Math.max(0, Math.min(1, st)) * obj.sliderPathLen,
              dist = 0
            let bx = path[0].x,
              by = path[0].y
            for (let j = 1; j < path.length; j++) {
              ...
              if (dist + seg >= target) {
                const f = seg > 0 ? (target - dist) / seg : 0
                bx = path[j - 1].x + dx * f
                by = path[j - 1].y + dy * f
                break
              }
  ...
  return (
    <div ref={containerRef} className={className} style={{ width: w, height: h, position: 'relative', ...style }}>
      <canvas
        ref={canvasRef}
